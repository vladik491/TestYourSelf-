...
[System.Serializable]
public class CharacterDataEasy
{
...
// Менеджер лёгкого уровня: формирование вопросов, обработка ответов, подсказок и модалок
public class EasyLevelManager : MonoBehaviour
{
    [Header("Данные персонажей (заполнить в инспекторе)")]
    public List<CharacterDataEasy> allCharacters = new List<CharacterDataEasy>();
    ...
    private List<CharacterDataEasy> currentQuestions;
    ...
    public static EasyLevelManager Instance;
    ...
    void Awake()
    {
        ...
        // Инициализируем список, если он пуст, чтобы избежать ошибок
        if (allCharacters == null) allCharacters = new List<CharacterDataEasy>();

        // ОЧИСТКА ПАМЯТИ: Удаляем из "списка использованных" индексы, которые больше не существуют
    ...
    void CreateQuestionSet()
    {
        ...
        currentQuestions = new List<CharacterDataEasy>();
        foreach (int idx in selected)
        {
            // Проверяем, что такой индекс действительно существует в списке персонажей
        ...
        // Страховка: если вдруг список оказался пуст (например, все выбранные индексы были битыми)
    ...
    void SetupQuestion()
    {
        ...
        var pool = allCharacters.Where(c => c.characterName != correctAnswer && !used.Contains(c.characterName))
                                .Select(c => c.characterName).ToList();
        ShuffleList(pool);
        int pi = 0;
        while (options.Count < 4 && pi < pool.Count)
        {
            options.Add(pool[pi]); used.Add(pool[pi]); pi++;
        }
        while (options.Count < 4) options.Add(correctAnswer);
